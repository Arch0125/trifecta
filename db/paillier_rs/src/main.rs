...
fn main() {
    ...
    let pt = 500u32;
    let ct = paillier_encrypt(&pubkey, &BigUint::from(pt));

    println!("ct u32: {:?}", ct.to_u64().unwrap());

    let ctu32 = BigUint::from(ct.to_u64().unwrap());



    println!("Decrypted: {:?}", paillier_decrypt(&privkey, &pubkey, &ctu32));
